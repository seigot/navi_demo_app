...
# FishAudio設定
FISHAUDIO_API_KEY = os.environ.get("FISHAUDIO_API_KEY", "XXX")
FISHAUDIO_MODEL_ID_EN = os.environ.get("FISHAUDIO_MODEL_ID_EN", "XXX")
FISHAUDIO_MODEL_ID_JA = os.environ.get("FISHAUDIO_MODEL_ID_JA", "XXX")
...
class NavigationController(QObject):
    ...
    def setLanguage(self, language):
        ...
        if language in ["ja", "en", "en_fishaudio", "ja_fishaudio"]:
            if language == "en_fishaudio":
                self.language = "en"
                self.tts_engine_type = "fishaudio"
                print(f"Changed language to {self.language}, TTS engine: {self.tts_engine_type}")
            elif language == "ja_fishaudio":
                self.language = "ja"
                self.tts_engine_type = "fishaudio"
                print(f"Changed language to {self.language}, TTS engine: {self.tts_engine_type}")
            else:
            ...
            if self.language == "en":
                ...
                # Try to set an English female voice
                try:
                    # Look for specific English female voices by ID pattern
                    english_female_voices = [v for v in voices if 
                    ...
                    if english_female_voices:
                        self.speech_engine.setProperty('voice', english_female_voices[0].id)
                        print(f"Setting English female voice: {english_female_voices[0].name} / {english_female_voices[0].id}")
                    elif english_voices:
                        self.speech_engine.setProperty('voice', english_voices[0].id)
                        print(f"Setting English voice: {english_voices[0].name} / {english_voices[0].id}")
                    else:
                        # Fallback to first voice
                        self.speech_engine.setProperty('voice', voices[0].id)
                except Exception as e:
                    print(f"Error setting English voice: {str(e)}")
            
            elif self.language == "ja":
                # Try to set a Japanese voice
                voices = self.speech_engine.getProperty('voices')
                try:
                    # Look for specific Japanese voices by ID pattern
                    ...
                    print(f"Error setting Japanese voice: {str(e)}")
            
            # Emit signal to update UI language
    ...
    def speakInstruction(self, instruction):
        """Provide voice guidance"""
        print(f"Speaking: {instruction}")
        
        try:
            print(f"TTS engine type: {self.tts_engine_type}")
            if self.tts_engine_type == "fishaudio":
                self.speak_with_fishaudio(instruction)
            else:
                # Use system TTS
                self.speech_engine.say(instruction)
                self.speech_engine.runAndWait()
        except Exception as e:
            print(f"Voice guidance error: {str(e)}")
            # Continue app even if voice fails
    
    def speak_with_fishaudio(self, text):
        """Use FishAudio API for text-to-speech"""
        try:
            print(f"Using FishAudio API for TTS: {text}")
            
            # 言語に応じてモデルIDを選択
            if self.language == "ja":
                model_id = FISHAUDIO_MODEL_ID_JA
                print(f"Using Japanese model ID: {model_id}")
            else:  # デフォルトは英語
                model_id = FISHAUDIO_MODEL_ID_EN
                print(f"Using English model ID: {model_id}")
            
            # Create a temporary file path
            temp_audio_file = os.path.abspath("temp_audio.mp3")
            print(f"Temporary audio file path: {temp_audio_file}")
            
            # Initialize FishAudio session
            print(f"FishAudio API Key: {FISHAUDIO_API_KEY[:5]}...{FISHAUDIO_API_KEY[-5:] if len(FISHAUDIO_API_KEY) > 10 else ''}")
            session = Session(FISHAUDIO_API_KEY)
            print("FishAudio session initialized")
            
            # Generate speech using FishAudio SDK
            print("Generating speech with FishAudio SDK...")
            with open(temp_audio_file, "wb") as f:
                ...
                for chunk in session.tts(TTSRequest(
                    ...
                    chunks.append(chunk)
                    f.write(chunk)
                print(f"Received {len(chunks)} chunks from FishAudio API")
            
            print(f"Audio file saved: {os.path.exists(temp_audio_file)}, size: {os.path.getsize(temp_audio_file) if os.path.exists(temp_audio_file) else 0} bytes")
            
            # Play the audio file using pygame (cross-platform)
            try:
                ...
                print(f"Pygame audio playback error: {str(audio_error)}")
                raise
